...
public static class AutoMap
{
    ...
    public static T? Clone<T>( T obj ) where T : notnull => obj.ToJson().FromJson<T>();
    ...
    /// <returns>
    ///  A new instance of <typeparamref name="TTo" /> with property values copied from the source object, or
    ///  <c>null</c> if mapping fails.
    /// </returns>
        ...
        foreach(PropertyInfo sourceProperty in sourceType.GetProperties()) {
            ...
            try {
                if(isCollection && (elementType is not null)) {
                    ...
                    object? targetCollection = processCollectionMethod.Invoke(elementType, [ value ]);
    ...
    ///  Only properties with the same name and compatible types are copied. If mapping fails, a new instance of the
    ///  target type is returned.
    /// </summary>
    ...
    /// <returns>
    ///  A new instance of <typeparamref name="TTo" /> with property values mapped from the source object, or a new
    ///  instance if mapping fails.
    /// </returns>
    ...
    /// <summary>
    ///  Processes a collection by creating a new instance of the same collection type and copying all elements from the
    ///  source collection. If the collection type cannot be instantiated, a <see cref="List{T}" /> is used as a
    ///  fallback.
    /// </summary>
    ...
    /// <returns>
    ///  A new collection of type <typeparamref name="TCollection" /> containing the copied elements from the source
    ///  collection.
    /// </returns>
